...
<section class="Hui-article-box">
	<nav class="breadcrumb"><i class="Hui-iconfont"></i> 首页
		...
		RBAC管理
		<span class="c-gray en">&gt;</span>
		用户列表 <a title="刷新" href="javascript:location.replace(location.href);" style="line-height:1.6em;margin-top:3px" class="btn btn-success radius r"><i class="Hui-iconfont"></i></a> </nav>
	<div class="Hui-article">
		<article class="cl pd-20">
			...
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				<span class="l"> <a class="btn btn-primary radius" href="<?php echo url('Jurisdiction/amstt_add'); ?>"><i class="Hui-iconfont"></i> 添加用户</a> </span>
				<!-- <span class="r">共有数据：<strong>54</strong> 条</span> -->
			</div>
			<table class="table table-border table-bordered table-bg">
				<thead>
					<tr>
						<th colspan="9" scope="col">用户列表</th>
					</tr>
					<tr class="text-c">
						...
						<tr class="text-c">
							...
							<td>
								<?php if($key=='0'){?>
									管理员权限
									<?php }else{?>
									<a href="<?php echo url('Jurisdiction/user_role'); ?>?userId=<?= $val['userId']?>">给用户添加角色</a>
								<?php }?>
							</td>
							...
							<?php if(\think\Session::get('name')=='KevinJia'){?>
										<a style="text-decoration:none" class="ml-5" href="<?php echo url('Jurisdiction/amstt_update'); ?>?userId=<?= $val['userId']?>" title="编辑">
											<i class="Hui-iconfont"></i>
										</a> 
										<a style="text-decoration:none" href="<?php echo url('Jurisdiction/amstt_del'); ?>?userId=<?= $val['userId']?>" title="删除">
											<i class="Hui-iconfont"></i>
										</a>
									<?php }else{?>
									不可操作
